...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.send_btn.place(x=730, y=550)

        # Для меню
        self.label = None
        self.entry = None

        # Запуск адаптивного UI
    ...
    def send_message(self):
        ...
        if text:
            ...
            label = CTkLabel(self.chat_field,self.message_entry.delete(0, 'end'))

    # -------------------------------
    def adaptive_ui(self):
        """Адаптивне оновлення розмітки"""
        self.menu_frame.configure(height=self.winfo_height())
        self.chat_field.place(x=self.menu_frame.winfo_width() + 10, y=50)
        self.chat_field.configure(width=self.winfo_width() - self.menu_frame.winfo_width() - 20,
                                  height=self.winfo_height() - 120)
        self.send_btn.place(x=self.winfo_width() - 60, y=self.winfo_height() - 50)
        self.message_entry.place(x=self.menu_frame.winfo_width() + 10, y=self.winfo_height() - 50)
        self.message_entry.configure(width=self.winfo_width() - self.menu_frame.winfo_width() - 90)
